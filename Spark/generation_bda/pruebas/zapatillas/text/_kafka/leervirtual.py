...
def leerConSpark():
    ...
    try:
        ...
        df = spark.read.option("header", "true").csv("s3a://my-local-bucket/txt_zapatillasConsumer2_csv")
        df.show()
        
        #df = spark.read.json("s3a://my-local-bucket/txt_zapatillasConsumer_json")        
        #df.show()
